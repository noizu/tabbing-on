#!/bin/zsh
# shell/tabbing.zsh — Zsh-specific tabbing-on command definitions
#
# Sources POSIX-compatible libs, then defines zsh functions that use
# zsh features (1-based arrays, [[ ]], precmd hooks).
#
...
#
# Examples:
#   tabbing-on "My Task" --highlight green "doing stuff" --pri 2
#   tabbing-on "My Task" -h green -p 2 "doing stuff"
#   tabbing-on -blue "My Task" -pri2 "doing stuff"
#   tabbing-on -rocket "My Task" "deploying"
#   tabbing-on   (no args: display current state)
# ---------------------------------------------------------------------------
tabbing-on() {
  ...
  if [[ $has_highlight -eq 1 ]]; then
    if ! _tabbing_color_code "$opt_highlight" >/dev/null 2>&1; then
      _tabbing_color_code "$opt_highlight"  # print error
      return 1
    fi
  ...
  if [[ $has_emoji -eq 1 ]]; then
    if ! _tabbing_emoji_lookup "$opt_emoji" >/dev/null 2>&1; then
      _tabbing_emoji_lookup "$opt_emoji"  # print error
      return 1
    fi
  ...
  # Positional args set title/status
  if [[ ${#positional[@]} -ge 1 ]]; then
    export TAB_TITLE="${positional[1]}"
    if [[ ${#positional[@]} -ge 2 ]]; then
      export TAB_STATUS="${positional[2]}"
    else
      unset TAB_STATUS
    fi
  fi
  ...
  # Handle recording flags (must come last as record_start spawns sub-shell)
...
#   Update status portion of tab title, re-render
# ---------------------------------------------------------------------------
tabbing-status() {
...
#
# Usage:
#   tabbing-todo "title" [-m "desc"] [--emoji NAME] [--urgency N]
#   tabbing-todo                      (list todos)
#   tabbing-todo -n | --switch        (interactive switch)
#   tabbing-todo --done [id]          (mark done)
#   tabbing-todo --record/--continue  (recording control)
# ---------------------------------------------------------------------------
tabbing-todo() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --done)
        do_done=1
        # Check if next arg is an ID (number)
        if [[ -n "${2:-}" && "$2" =~ ^[0-9]+$ ]]; then
          done_id="$2"; shift 2
        else
          shift
...
#
# Usage:
#   tabbing-report                  (current tab, ASCII chart)
#   tabbing-report --mermaid        (current tab, Mermaid syntax)
#   tabbing-report --tab TAB_ID     (specific tab)
...
# ---------------------------------------------------------------------------
# Optional: auto-render on each prompt (precmd hook)
# Keeps the terminal title in sync if the shell title gets overwritten
# ---------------------------------------------------------------------------
_tabbing_precmd() {
  if [[ -n "${TAB_TITLE:-}" ]]; then
    _tabbing_render
...
# Uncomment to enable auto-render on each prompt:
# precmd_functions+=(_tabbing_precmd)
